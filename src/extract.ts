...
export const BUILTIN_RULES: ExtractRule[] = [
 ...
 * built-in rules.
 */
export function matchRule(hostname: string, rules: readonly ExtractRule[]): ExtractRule | undefined {
  const host = normalizeHost(hostname)
  let best: ExtractRule | undefined
  let bestLen = -1
...
function pickContent(document: any, rule: ExtractRule | undefined): any | undefined {
  ...
  for (const sel of candidates) {
    try {
      const el = document.querySelector(sel)
      if (el && (el.textContent ?? '').trim().length > 60) return el
    } catch { /* invalid selector */ }
  }
  const body = document.body
  if (body) {
    const text = (body.textContent ?? '').trim()
    if (text.length > 60) return body
  }
  return undefined
}

/** Try JSON-LD articleBody as the extraction source. */
function jsonLdBody(document: any): string | undefined {
  ...
  for (const s of scripts) {
    try {
      ...
      for (const item of list) {
        ...
        if (typeof body === 'string' && body.trim().length > 60) return body
      }
    } catch { /* not JSON */ }
  }
  return undefined
    ...
    if (content) {
      if (rule?.removeSelectors) {
        for (const sel of rule.removeSelectors) {
          try { content.querySelectorAll(sel).forEach((el: any) => el.remove()) } catch { /* ignore */ }
        }
      }
      if (!rule) {
...
export function extractResultLinks(html: string): { title: string; url: string }[] {
  ...
  while ((m = anchorRe.exec(html)) !== null) {
    ...
    if (!/^https?:\/\//i.test(href) || inner.length < 4) continue
