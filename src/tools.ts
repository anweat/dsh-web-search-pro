...
export function registerTools(deps: ToolDeps): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Search a specific platform: github (gh CLI), bilibili (bili-cli), youtube (yt-dlp), v2ex (sov2ex), xiaohongshu/twitter/reddit/instagram/facebook (opencli browser session), or rss (any feed URL). Results are persisted to the search history.',
    parameters: {
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as { platform: string; sources: { url: string; title?: string; snippet?: string; publishedAt?: string }[]; engine: string; fromCache: boolean }
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        ...
        additionalProperties: false,
        properties: {
          records: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { kind: { type: 'string' }, query: { type: 'string' }, engine: { type: 'string' }, platform: { type: 'string' }, url: { type: 'string' }, status: { type: 'string' }, ts: { type: 'string' } } } },
        },
      },
      render: (_args, value) => {
        const v = value as { records: { kind: string; query?: string; engine?: string; platform?: string; url?: string; status: string; ts: string }[] }
        if (!v.records.length) return [{ type: 'text', text: 'No history records found.' }]
        const lines = v.records.map(r => {
          const what = r.query ?? r.url ?? ''
          const via = r.engine ?? r.platform ?? ''
          return '- [' + r.kind + '] ' + r.ts + ' · ' + (r.status === 'ok' ? 'ok' : r.status) + ' · ' + what + (via ? ' (' + via + ')' : '')
        })
    ...
    async execute(args) {
      ...
      const records = store.listQueries({ ...kind ? { kind } : {}, ...args.query ? { query: args.query } : {}, limit: args.limit ?? 20 })
      return {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (action === 'upsert') {
        if (!args.contentSelectors) throw new Error('contentSelectors is required for upsert')
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (v.install) parts.push('安装结果 exit=' + v.install.code + (v.install.timedOut ? ' (超时)' : '') + '\n' + (v.install.stderr || v.install.stdout).slice(0, 2000))
        return [{ type: 'text', text: parts.join('\n') }]
      },
    },
