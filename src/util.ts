...
export function resolvePlaywright(modulePath?: string): any {
  if (cachedPlaywright) return cachedPlaywright
  const anchors: string[] = []
  if (modulePath) anchors.push(path.join(modulePath, 'package.json'))
...
export function decodeRedirectUrl(href: string): string {
  ...
  if (g) {
    try { return decodeURIComponent(g[1]!) } catch { /* fall through */ }
  }
  return href
}
