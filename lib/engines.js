...
import { PLATFORM_SEARCH_SPECS } from "./platform-search.js";
...
export function playwrightPlatformEngine(platform, deps) {
    ...
    return {
        ...
        async search(query, count, signal) {
            ...
            if (!sources.length) {
                throw new EngineError(builtin.label + ' 未取到结果：该平台需要浏览器登录态（复用你已登录的浏览器）。运行 node scripts/save-login.mjs 登录一次并设置 playwright.storageStatePath；或到 $DSH_HOME/settings.yaml 的 platformRules.' + platform + ' 微调结果选择器。', 'ENGINE_EMPTY', false);
            }
            return { sources };
        },
    };
...
export const PLATFORM_IDS = ['github', 'bilibili', 'youtube', 'v2ex', 'xiaohongshu', 'twitter', 'reddit', 'instagram', 'facebook', 'rss', 'zhihu', 'weibo', 'douban', 'tieba', 'douyin', 'kuaishou', 'arxiv', 'pubmed'];
