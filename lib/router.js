...
export class SearchRouter {
            ...
            this.backends.register({
                ...
                probe: () => {
                    try {
                        ...
                        if (id === 'exa' && this.ctx.get('credentials'))
                            return { available: true, reason: 'credential resolution deferred until execution' };
    ...
    async search(opts) {
        ...
        const cacheKey = createSearchCacheKey({ query, engines: ids, count, multi: opts.multi, ...opts.exa ? { exa: opts.exa } : {} });
        ...
        if (opts.multi) {
            ...
            try {
                const selected = await this.backends.runSelected({ query, count, signal, skipSeam: opts.skipSeam ?? false, ...opts.exa ? { options: { exa: opts.exa } } : {} }, { preferred: ids });
