...
export class Store {
    ...
    queryById(id) {
        return this.db.prepare('SELECT * FROM queries WHERE id = ?').get(id);
    }
    /** Fresh page snapshot by URL, or undefined. */
    getPage(url, ttlSeconds) {
        const row = this.db.prepare(`SELECT ${PAGE_COLUMNS} FROM pages WHERE url = ? AND fetched_at > ? ORDER BY fetched_at DESC LIMIT 1`).get(url, new Date(Date.now() - ttlSeconds * 1000).toISOString());
        return row;
    }
    savePage(input) {
    ...
    clearCache(opts) {
        ...
        if (opts.olderThanDays === undefined) {
            ...
            const p = this.db.prepare(`SELECT COUNT(*) AS c FROM pages WHERE query_id IN (SELECT id FROM queries WHERE ${predicate}) OR (query_id IS NULL AND fetched_at < ?)`).get(...params, since);
    ...
    topEngines(limit = 8) {
        return this.db.prepare('SELECT engine, COUNT(*) AS count FROM queries WHERE engine IS NOT NULL GROUP BY engine ORDER BY count DESC LIMIT ?').all(limit);
    }
    /** Most-frequent queries, desc. */
    topQueries(limit = 8) {
        return this.db.prepare('SELECT query, COUNT(*) AS count FROM queries WHERE query IS NOT NULL GROUP BY query ORDER BY count DESC LIMIT ?').all(limit);
    }
    /** Per-kind record counts. */
    kindCounts() {
        return this.db.prepare('SELECT kind, COUNT(*) AS count FROM queries GROUP BY kind ORDER BY count DESC').all();
    }
    stats() {
