 ...
 */
import { parseDocument, stripTags } from "./util.js";
...
function rawText(node) {
    ...
    if (tag === 'PRE' || tag === 'CODE') {
        ...
        if (typeof source === 'string' && source.length > 0) {
            try {
                const fragment = parseDocument(source);
                const nodes = fragment.body?.childNodes ?? [];
                let out = '';
...
export function extractText(html, url, rules, maxChars = 200_000) {
    ...
    if (ogContent.trim().length > title.length)
        title = ogContent.trim();
